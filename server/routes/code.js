...
// Route to save user note
router.post('/save-note', async (req, res) => {
  const { jobId, userNote } = req.body;

  try {
    const job = await Job.findById(jobId);
    if (!job) return res.status(404).json({ error: 'Job not found' });

    job.userNote = userNote;
    await job.save();

    res.status(200).json({ message: 'User note saved successfully' });
  } catch (error) {
    res.status(500).json({ error: 'Internal server error' });
  }
});

// Route to fetch user note
router.get('/fetch-note/:jobId', async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ userNote: job.userNote });
  } catch (error) {
    res.status(500).json({ error: 'Internal server error' });
  }
});
